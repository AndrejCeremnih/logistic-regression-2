...
const (
	...
	epochs              = 100000
	printEveryNthEpochs = 10000
	lrW                 = 1e-3
	lrB                 = 1e-1
	scale               = 10

	funcType = 1 // 1 is linear, 2 is quadratic, 3 is cubic
)
...
func dCost(inputs [][]float64, y, p []float64) (dw []float64, db float64) {
	dw = make([]float64, len(polynomial(0, 0, funcType)))
...
func polynomial(x1, x2 float64, funcType int) (res []float64) {
...
func main() {
	...
	for i := range inputs {
		inputs[i] = polynomial(inputs[i][0], inputs[i][1], funcType)
	}
	...
	go func() {
		learningRate := 1e-3
		w := make([]float64, len(polynomial(0, 0, funcType)))
		...
		for i := 0; i <= epochs; i++ {
			...
			if i%printEveryNthEpochs == 0 {
				boundPlot := decBoundPlot{
					...
					f: func(c, r int) float64 {
						x := polynomial(float64(c), float64(r), funcType)
						return p(x, w, b)
					},
				}
